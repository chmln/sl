...
EXAMPLES:
    symlink /target/path to current directory
    $ sl /target/path
...
fn main() -> Result<()> {
    ...
    if let Some(pointing_to) = pointing_to {
        ...
        let pointing_to = link_name.canonicalize()?;
        let link_name = pointing_to.file_name().ok_or(Error::NoFileName)?;
